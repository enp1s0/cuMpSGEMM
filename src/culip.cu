...
const std::string CUMPSGEMM_ENABLE_CULIP_PROFILING_ENV_NAME = "CUMPSGEMM_ENABLE_CULIP_PROFILING";

void cumpsgemm::CULiP::record_timestamp(void *tm_timestamp) {
	struct timespec *tm_ptr = (struct timespec *)tm_timestamp;
	clock_gettime(CLOCK_MONOTONIC, tm_ptr);
}

void cumpsgemm::CULiP::print_profile_result(void *profile_result_ptr) {
	const auto profile_result =
	    *((cumpsgemm::CULiP::profile_result*)profile_result_ptr);
	...
	printf("[%s][%s] %luns\n", CULIP_RESULT_PREFIX.c_str(), profile_result.function_name, elapsed_time_us);
	fflush(stdout);
}

void cumpsgemm::CULiP::launch_function(cudaStream_t cuda_stream, void (*fn)(void*), void* const arg) {
	cudaStreamSynchronize(cuda_stream);
	fn(arg);
	cudaStreamSynchronize(cuda_stream);
}

bool cumpsgemm::CULiP::is_profiling_enabled() {
...
const char* cumpsgemm::CULiP::get_cublasComputeType_t_string(const cublasComputeType_t compute_type) {
	switch(compute_type) {
...
const char* cumpsgemm::CULiP::get_cublasOperation_t_string(const cublasOperation_t op) {
	switch(op) {
	case CUBLAS_OP_N:
		return "N";
	case CUBLAS_OP_T:
