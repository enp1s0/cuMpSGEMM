...
const std::string CUMPSGEMM_ENABLE_CULIP_PROFILING_ENV_NAME =
    "CUMPSGEMM_ENABLE_CULIP_PROFILING";
...
void cumpsgemm::CULiP::launch_function(cudaStream_t cuda_stream,
...
#define CULiP_CUBLAS_COMPUTE_T_CASE_STRING(compute_type)                       \
  case compute_type:                                                           \
    return #compute_type
const char *cumpsgemm::CULiP::get_cublasComputeType_t_string(
...
const char *
cumpsgemm::CULiP::get_cublasOperation_t_string(const cublasOperation_t op) {
  switch (op) {
  case CUBLAS_OP_N:
    return "N";
  case CUBLAS_OP_T:
